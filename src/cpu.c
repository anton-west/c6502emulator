...
void cpu_reset(Processor *cpu) {
    ...
    cpu->sp = 0xFD;
    cpu->status_reg = 0 | flag_U | flag_I;   //unused flag is always high when status reg is pushed to stack, can be high all the time as it has no effect on cpu operation
